...
class Workout extends React.Component {
	...
	componentDidMount() {
		fetch(`http://workout.beynum.com/api/workoutload`, { credentials: "include" })
	...
	addWorkout = event => {
		...
		fetch(`http://workout.beynum.com/api/workoutsave`, { method: "post", headers: {"Content-Type": "application/json"}, credentials: "include", body: JSON.stringify({ workout: newWorkout }) })
			.then(response => response.json())
			.then(data => {
	...
	deleteWorkout = workoutIndex => event => {
		fetch(`http://workout.beynum.com/api/workoutdelete?id=${this.state.workouts[workoutIndex].id}`, { credentials: "include" })
			.then(response => response.json())
			.then(data => {
				this.setState(({workouts}) => ({
					workouts: [
	...
	saveWorkout = workoutIndex => event => {
		fetch(`http://workout.beynum.com/api/workoutsave`, { method: "post", headers: {"Content-Type": "application/json"}, credentials: "include", body: JSON.stringify({ workout: this.state.workouts[workoutIndex] }) })
			.then(response => response.json())
			.then(data => {
				this.setState(({workouts}) => ({
					workouts: workouts.map((workout, itemIndex) => ({
